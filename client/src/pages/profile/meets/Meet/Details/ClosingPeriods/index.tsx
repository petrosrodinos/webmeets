...
const ClosingPeriods: FC<Step3Props> = ({ values, meetId }) => {
  ...
  const { mutate: addPeriodMutation, isLoading: isAddingPeriod } = useMutation(
    (data: AddClosingPeriod) => {
      return addClosingPeriod(data);
    }
  );

  const { mutate: editPeriodMutation, isLoading: isEditingPeriod } = useMutation(
    (data: EditClosingPeriod) => {
      return editClosingPeriod(data);
    }
  );

  const { mutate: deletePeriodMutation, isLoading: isDeletingPeriod } = useMutation(
    (data: DeleteClosingPeriod) => {
