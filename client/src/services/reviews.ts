...
export const getMeetReviews = async (meetId: string): Promise<Review[]> => {
  try {
    const result = await axios.get(`${API_URL}reviews/${meetId}`);
    const formattedData = result.data.map((review: any) => formatReview(review));
...
export const getReviews = async (query: { [key: string]: string }): Promise<Review[]> => {
  try {
